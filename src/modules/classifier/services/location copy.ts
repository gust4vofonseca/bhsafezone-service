const neighborhoodAndRegion: Record<string, string> = {
  'AARÃO REIS': 'NORTE',
...
export function locateARegion(texto: string): string | null {
  const textoUpper = texto.toUpperCase();

  for (const neighborhood in neighborhoodAndRegion) {
    if (textoUpper.includes(neighborhood.toUpperCase())) {
      return neighborhoodAndRegion[neighborhood];
    }
  }
